                    ...
                    <div className="space-y-4 px-4">
                      <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                          <h3 className="font-serif text-lg font-semibold group-hover:text-gold transition-colors leading-[1.2] uppercase tracking-tight truncate block flex-grow">{product.name}</h3>
                          {(() => {
                            const now = new Date();
                            const preOrderEndsAt = product.preOrderEndsAt ? new Date(product.preOrderEndsAt) : null;
                            const limitedTimeEndsAt = product.limitedTimeEndsAt ? new Date(product.limitedTimeEndsAt) : null;
                            ...
                            if (currentPrice < product.price) {
                              return (
                                <div className="flex items-baseline gap-3">
                                  <span className="text-base text-ink font-serif font-semibold tracking-tight tabular-nums whitespace-nowrap">{formatPrice(currentPrice)}</span>
                                  <span className="text-[10px] text-muted line-through opacity-30 font-bold tabular-nums whitespace-nowrap">{formatPrice(product.price)}</span>
                                </div>
                              );
                            }
                            return <span className="text-base text-ink font-serif font-semibold tracking-tight tabular-nums whitespace-nowrap">{formatPrice(product.price)}</span>;
                          })()}
                        </div>
                      </div>
